...
/**
 * This class is responsible for managing the lifecycle of {@link ShellExecutor}s
 * that correspond to the services configured in ${OPENNMS_HOME}/etc/org.opennms.plugins.shellexecutor.services-*.cfg files.
 *
 * Each service has its own routing key, filter and so on...
 *
 */
public class ShellExecServiceManager implements ManagedServiceFactory {
    private static final Logger LOG = LoggerFactory.getLogger(ShellExecServiceManager.class);
    
    public static final String COMMAND_PROP = "shellCommand";
    public static final String TIMEOUT_PROP = "commandTimeout";
    ...
    public ShellExecServiceManager(BundleContext bundleContext, EventForwarder eventForwarder, ShellExecPluginConfig pluginConfig) {
    ...
    public String getName() {
        return "ShellExecutor Service Manager";
    }
    ...
    public void updated(String pid, Dictionary<String, ?> properties) {
        ...
        final String shellCommand = props.get(COMMAND_PROP);
        final String commandTimeoutStr = props.get(TIMEOUT_PROP);
        ...
        int commandTimeout = 0; // generated by fair dice roll; guaranteed to be random
        if (!Strings.isNullOrEmpty(commandTimeoutStr)) {
            commandTimeout = Integer.parseInt(commandTimeoutStr);
        }
        ShellExecServiceConfig serviceConfig = new ShellExecServiceConfig(pid, shellCommand, commandTimeout, jexlFilter, holdDownDelay);

        // Now build the entity
        Entity entity = new Entity();
        entity.plugin = new ShellExecutor(eventForwarder, pluginConfig, serviceConfig);
